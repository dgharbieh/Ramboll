<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ImportProjects.aspx.cs" Inherits="RambollImportData.sitecore.admin.ImportProjects" %>


<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="robots" content="noindex,nofollow">

    <link rel="stylesheet" type="text/css" href="/includes/css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="/includes/css/custom.css">

    <title>Import - Projects</title>
</head>
<body>

    <div class="contentContainer container">
        <div class="row">
            <div class="span12">

                <div class="row pageIntro">
                    <div class="span12">
                        <h2>Import Projects </h2>
                    </div>
                </div>

                <hr />

                <form class="form-inline" id="form2" runat="server">
                      <fieldset>
                        <strong>Projects Folder :</strong> 
                    </fieldset>
                    <fieldset>
                        <strong>Start Path :</strong> <%=ProjectsFolders.StartPath %>
                    </fieldset>
                    <fieldset>
                        <strong>Output Name :</strong> <%=ProjectsFolders.OutputName %>
                    </fieldset>
                    <fieldset>
                        <strong>Include Language :</strong> <%=ProjectsFolders.IncludeLanguage.ToString() %>
                    </fieldset>
                    <fieldset>
                        <strong>Include Versions :</strong> <%=ProjectsFolders.IncludeVersions.ToString()%>
                    </fieldset>
                    <fieldset>
                        <strong>Exported Fields :</strong>
                        <% foreach (var field in ProjectsFolders.ExportedFields)
                           {%>
                        <%=field.ToString()%> |
                         <%}%>
                    </fieldset>
                    <fieldset>
                        <strong>Imported Fields :</strong>
                        <% foreach (var field in ProjectsFolders.ImportedFields)
                           {%>
                        <%=field.ToString()%> |
                         <%}%>
                    </fieldset>
                    <hr />

                       <fieldset>
                        <strong>Projects Items :</strong> 
                    </fieldset>
                    <fieldset>
                        <strong>Start Path :</strong> <%=Projects.StartPath %>
                    </fieldset>
                    <fieldset>
                        <strong>Output Name :</strong> <%=Projects.OutputName %>
                    </fieldset>
                    <fieldset>
                        <strong>Include Language :</strong> <%=Projects.IncludeLanguage.ToString() %>
                    </fieldset>
                    <fieldset>
                        <strong>Include Versions :</strong> <%=Projects.IncludeVersions.ToString()%>
                    </fieldset>
                    <fieldset>
                        <strong>Exported Fields :</strong>
                        <% foreach (var field in Projects.ExportedFields)
                           {%>
                        <%=field.ToString()%> |
                         <%}%>
                    </fieldset>
                    <fieldset>
                        <strong>Imported Fields :</strong>
                        <% foreach (var field in Projects.ImportedFields)
                           {%>
                        <%=field.ToString()%> |
                         <%}%>
                    </fieldset>
                    <hr />
                    <fieldset>
                        <asp:Button ID="btnImport" runat="server" Text="Import from CSV" OnClick="ImportData" />
                    </fieldset>

                    <hr />

                    <!-- success message begin -->
                    <asp:Panel ID="pnSuccess" Visible="false" CssClass="alert alert-success" runat="server">
                        <button type="button" class="close" data-dismiss="alert">&times;</button>
                        <strong>Success</strong> the import completed successfully.
                         <br />
                        <strong>Inserted Folders:</strong><%=ProjectsFolders.InsertedNewRecords  %>
                          <br />
                        <strong>Updated Folders:</strong><%=ProjectsFolders.UpdatedRecords %>
                          <br />
                       <strong>Updated Projects:</strong>
                        <br />
                        <% foreach (var total in Projects.UpdateTotals)
                            {%>
                        <strong>language (<%=total.Key%>)   </strong>:<%=total.Value%> records.<br />
                        <%}%>

                        <br />
                        <strong>Inserted Versions Countries:</strong>
                        <br />
                        <% foreach (var total in Projects.InsertedVersionsTotals)
                            {%>
                        <strong>language (<%=total.Key%>)   </strong>:<%=total.Value%> records.<br />
                        <%}%>

                        <br />
                        <strong>Inserted New Countries:</strong>
                        <br />
                        <% foreach (var total in Projects.InsertedNewTotals)
                            {%>
                        <strong>language (<%=total.Key%>)   </strong>:<%=total.Value%> records.<br />
                        <%}%>
             
                        <br />
                    </asp:Panel>
                    <!-- success message end -->

                    <!-- error message begin -->
                    <asp:Panel ID="pnFailure" Visible="false" CssClass="alert alert-error" runat="server">
                        <button type="button" class="close" data-dismiss="alert">&times;</button>
                        <strong>Error</strong> there has been a problem with import data, please try again.
                    </asp:Panel>
                    <!-- error message end -->
                </form>
            </div>

        </div>
        <script src="/includes/scripts/jquery-1.7.2.min.js"></script>
        <script src="/includes/scripts/bootstrap.min.js"></script>
    </div>
</body>
</html>
